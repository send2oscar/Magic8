...
export function AdminCreditPaymentControls() {
  ...
  const submitPolicy = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Credit policy saved for all users. New successful generations and package prices now use these settings.");
    } catch (error) {
  ...
  const submitPackage = async (id: number) => {
    ...
    try {
      ...
      await invalidateFinance();
      toast.success("Credit package saved for all users.");
    } catch (error) {
  ...
  const addPackage = async () => {
    ...
    try {
      ...
      await invalidateFinance();
      toast.success("New 100-credit package added for all users. Edit its values below as needed.");
    } catch (error) {
    ...
    <div className="space-y-6">
      <Card className="hud-frame bg-card/50 p-6">
        <div className="mb-5 flex flex-wrap items-start justify-between gap-3">
          <div className="flex items-center gap-2"><Settings2 className="h-5 w-5 text-accent" /><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">CREDIT POLICY</h2><span className="rounded border border-accent/50 bg-accent/10 px-2 py-0.5 text-[10px] font-bold tracking-wide text-accent">ALL USERS</span></div><p className="mt-1 text-xs text-muted-foreground">These site-wide values are enforced by the server for every user. Credits are deducted only after a successful generation.</p></div></div>
          <span className="rounded border border-accent/50 bg-accent/10 px-2 py-1 text-xs font-semibold text-accent">PAYPAL LIVE</span>
        </div>
        {policy.isLoading ? <div className="flex justify-center p-6"><LoaderCircle className="h-5 w-5 animate-spin text-accent" /></div> : policy.isError || !policy.data ? <p className="text-sm text-destructive">Unable to load the administrator credit policy.</p> : (
          <form onSubmit={submitPolicy} className="space-y-4">
            ...
            <Button type="submit" disabled={savePolicy.isPending} className="border border-accent bg-accent font-bold text-accent-foreground shadow-[0_0_20px_oklch(0.73_0.27_342_/_0.2)]">{savePolicy.isPending ? <LoaderCircle className="mr-2 h-4 w-4 animate-spin" /> : <CircleDollarSign className="mr-2 h-4 w-4" />} SAVE CREDIT POLICY</Button>
          </form>
        )}
      </Card>

      <Card className="hud-frame bg-card/50 p-6">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">FIXED CREDIT PACKAGES</h2><span className="rounded border border-accent/50 bg-accent/10 px-2 py-0.5 text-[10px] font-bold tracking-wide text-accent">ALL USERS</span></div><p className="mt-1 text-xs text-muted-foreground">Shared checkout choices shown to every user. The USD checkout amount is derived from the active policy and cannot be edited per package.</p></div><Button type="button" variant="outline" onClick={() => void addPackage()} disabled={savePackage.isPending} className="border-accent/60 text-accent hover:bg-accent/10"><Plus className="mr-2 h-4 w-4" /> ADD PACKAGE</Button></div>
      ...
      <Card className="hud-frame bg-card/50 p-6">
        <div className="mb-5 flex items-center gap-2"><ReceiptText className="h-5 w-5 text-accent" /><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-bold">PAYPAL PAYMENT RECORDS</h2><span className="rounded border border-accent/50 bg-accent/10 px-2 py-0.5 text-[10px] font-bold tracking-wide text-accent">ALL USERS</span></div><p className="mt-1 text-xs text-muted-foreground">All-user return-page capture records, including incomplete and failed PayPal checkouts.</p></div></div>
